...
function titleCursorY() { const rect = manifestoTitle.getBoundingClientRect(), screenY = rect.top - (innerWidth < 760 ? 52 : 72), distance = camera.position.z - .4, worldHeight = 2 * Math.tan(THREE.MathUtils.degToRad(camera.fov * .5)) * distance; return (.5 - screenY / innerHeight) * worldHeight; }
ScrollTrigger.create({ trigger: ".manifesto-wrap", start: "top top", end: () => ScrollTrigger.maxScroll(scroller), invalidateOnRefresh: true, onEnter: () => { scenePhase = 1; if (cursor3d) cursor3d.visible = false; if (manifestoCursor) manifestoCursor.visible = true; }, onEnterBack: () => { scenePhase = 1; if (cursor3d) cursor3d.visible = false; if (manifestoCursor) manifestoCursor.visible = true; }, onLeave: () => { scenePhase = 2; scene.background.copy(skyColor); bgMaterial.opacity = .38; setPixelTransition(0, "#07144d"); setStickerOpacity("end", .92); }, onLeaveBack: () => { scenePhase = 0; setStickerOpacity("end", 0); stars.material.uniforms.uOpacity.value = 0; starsNear.material.uniforms.uOpacity.value = 0; if (manifestoCursor) manifestoCursor.visible = false; }, onUpdate(self) {
  const p = self.progress;
  pixelTransition.classList.remove("protect-content"); scene.background.copy(blackColor); bgMaterial.opacity = 0; const endDots = Math.max(0, Math.min(1, (p - .91) / .09)); setPixelTransition(endDots, "#07144d");
